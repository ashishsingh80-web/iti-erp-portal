...
import { applyOcrImport, previewOcrImport, type OcrImportKind, type OcrImportPreview } from "@/lib/services/ocr-import-service";
  ...
  try {
    ...
    if (mode === "apply") {
      ...
      if (typeof previewRaw !== "string") {
        return NextResponse.json({ ok: false, message: "Preview payload is required for apply" }, { status: 400 });
      }

      const preview = JSON.parse(previewRaw) as OcrImportPreview;
