...
export async function GET(request: Request) {
  try {
    ...
    assertUserActionAccess(user, "prn", "view");
    const { searchParams } = new URL(request.url);
    const search = String(searchParams.get("search") || "").trim();
    const session = String(searchParams.get("session") || "").trim();
    const yearLabel = String(searchParams.get("yearLabel") || "").trim();
    const scvtVerificationStatus = String(searchParams.get("scvtVerificationStatus") || "").trim();
    const missingPrn = searchParams.get("missingPrn") === "1";
    const page = Math.max(Number(searchParams.get("page") || "1"), 1);
    const pageSize = Math.min(100, Math.max(Number(searchParams.get("pageSize") || "25"), 10));

    const where = {
      ...(missingPrn ? { prnNumber: null } : {}),
      ...(scvtVerificationStatus
        ? { verificationStatus: scvtVerificationStatus as VerificationStatus }
        : {}),
              ...
              ...(search
                ? {
                    OR: [
                      ...
                      { prnScvtRecord: { prnNumber: { startsWith: search, mode: "insensitive" as const } } }
                    ]
                  }
                : {})
            }
    ...
    const [rows, total, byVerification] = await Promise.all([
    ...
    return NextResponse.json({
      ...
      rows,
      counts: {
        verification: byVerification.reduce<Record<string, number>>((acc, row) => {
          acc[row.verificationStatus] = row._count._all;
          return acc;
        }, {}),
        missingPrn: await prisma.prnScvtRecord.count({ where: { prnNumber: null } })
      },
      pagination: {
        page,
        pageSize,
    ...
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "Unable to load PRN queue" },
