  ...
  try {
    ...
    return NextResponse.json({
      ...
      trades: trades.map((item) => ({
        ...
        ncvtScvt: item.ncvtScvt || "",
        standardFees: item.standardFees?.toString() || "",
        isActive: item.isActive
      }))
    });
  } catch (error) {
    return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "Unable to load trades" }, { status: 400 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "Unable to create trade" }, { status: 400 });
  }
}
