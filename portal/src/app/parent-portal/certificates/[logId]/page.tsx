...
export default async function ParentPortalCertificatePage({ params }: { params: Promise<{ logId: string }> }) {
  const user = await requireParentUser();
  const { logId } = await params;
  const log = await getCertificatePrintData(logId);
  if (!log || log.studentId !== user.studentId) notFound();
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm print:rounded-none print:border-none print:shadow-none">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Parent Portal Certificate</p>
        ...
        <PortalPrintActions backHref="/parent-portal" />
      </section>
    </div>
  );
}
