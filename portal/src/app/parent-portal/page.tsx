...
function buildPortalAlerts(profile: Awaited<ReturnType<typeof getStudentProfile>> extends infer T ? NonNullable<T> : never) {
  ...
  if (profile.scholarship?.status === "QUERY" || profile.scholarship?.status === "UNDER_PROCESS") {
    alerts.push({
      ...
      message: `Scholarship status is ${profile.scholarship.status}. Please coordinate with the institute office.`
    });
  }
  ...
  if (profile.supportCases.some((item) => item.status !== "RESOLVED" && item.status !== "CLOSED")) {
    alerts.push({
      ...
      message: "There are support cases still under office review."
    });
  }
  ...
  if (!alerts.length) {
    alerts.push({
      ...
      message: "The student record currently looks stable in the portal."
    });
  }
...
export default async function ParentPortalPage() {
  ...
  const profile = await getStudentProfile(parent.studentId);
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <section className="surface rounded-[2rem] bg-gradient-to-br from-slate-950 via-orange-900 to-emerald-500 p-8 text-white">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-white/75">{t(lang, "Parent Portal")}</p>
            <h1 className="mt-3 font-serif text-4xl font-semibold tracking-tight">{parent.parentName}</h1>
            <p className="mt-2 text-sm text-white/85">
              {parent.relation} of {profile.fullName} • {profile.studentCode}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <LanguageSelector currentLanguage={lang} />
            <ParentLogoutButton className="rounded-full border border-white/30 px-4 py-2 text-sm font-semibold text-white" />
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500">{t(lang, "Student")}</p>
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Trade</p>
          <p className="mt-3 text-lg font-semibold text-slate-900">{profile.tradeName}</p>
        </div>
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
          ...
          {profile.prnScvt ? (
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <p>Admission Status: <span className="font-semibold text-slate-900">{profile.prnScvt.admissionStatus || "-"}</span></p>
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
          ...
          {profile.examStatus ? (
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              ...
              <p>Hall Ticket: <span className="font-semibold text-slate-900">{profile.examStatus.hallTicketIssuedOn ? "Issued" : "Pending"}</span></p>
            </div>
          ) : (
            ...
            <div className="mt-4 space-y-3">
              {profile.certificates.slice(0, 5).map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm">
                  ...
                  <Link className="font-semibold text-emerald-800 underline-offset-4 hover:underline" href={`/parent-portal/certificates/${item.id}`}>
                    Open Certificate
                  </Link>
                </div>
              ))}
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
          ...
          {profile.undertaking ? (
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              ...
              <Link className="inline-flex rounded-full border border-emerald-300 px-4 py-2 font-semibold text-emerald-800" href="/parent-portal/undertaking">
                Open Undertaking
              </Link>
            </div>
          ) : (
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5">
          ...
          <SupportRequestForm endpoint="/api/parent-portal/support" lang={lang} />
        </div>
