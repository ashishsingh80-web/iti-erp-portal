...
export function IdCardPrintActions({
  studentId,
  printCount = 0,
  status = "ACTIVE"
}: {
  ...
  async function handlePrint() {
    ...
    const response = await fetch("/api/id-cards/print", {
      ...
      body: JSON.stringify({ entityType: "student", entityId: studentId, reason })
    });
    if (!response.ok) {
  ...
  return (
    <div className="mt-6 flex flex-wrap gap-3 print:hidden">
      ...
      <Link className="rounded-2xl bg-slate-100 px-5 py-3 text-sm font-semibold text-slate-700" href={`/students/${studentId}`}>
        Back To Student
      </Link>
    </div>
  );
