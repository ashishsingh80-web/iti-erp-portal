  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="surface p-6">
          ...
          <div className="mt-3 grid gap-4 xl:grid-cols-2">
            {data.seatView.length ? (
              data.seatView.map((item) => (
                <article key={`${item.tradeValue}-${item.session}-${item.yearLabel}`} className="rounded-3xl border border-slate-100 bg-slate-50/80 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <a className="text-sm font-semibold text-slate-950 hover:text-emerald-700" href={buildAdmissionLink(item)}>
                        {item.tradeName}
                      </a>
                      <a className="mt-1 block text-xs text-slate-500 hover:text-emerald-700" href={buildAdmissionLink(item)}>
                        {item.instituteCode} • {item.session} • {item.yearLabel}
                      </a>
                      <a className="mt-2 block text-xs font-medium text-slate-600 hover:text-emerald-700" href={buildAdmissionLink(item, "assigned")}>
                    ...
                    <div className="rounded-2xl bg-white px-3 py-2 text-right">
                      <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Vacant Seats</p>
        ...
        <div className="mt-6 overflow-x-auto rounded-3xl border border-slate-100" id="admission-list">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500">
              <tr>
...
function MetricCard({
  ...
  return (
    <article className={`rounded-3xl px-5 py-4 ${toneMap[tone]}`}>
